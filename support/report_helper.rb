...
module ReportHelper
  def generate_file_path_and_name(s)
    project_dir = File.expand_path(File.dirname(__FILE__) + '/../')
    time_fixed ||= Time.now.strftime('%Y-%m-%d_%H-%M-%S')
    time = Time.now.strftime('%Y-%m-%d_%H-%M-%S')
    feature = s.full_description.underscore_all
    report_folder = File.join("#{project_dir}/build", time_fixed, feature)
    report_file_name = "#{s.description.underscore_all}_#{time}"
